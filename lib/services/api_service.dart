...
class ApiService {
  ...
  Future<List<Movie>> fetchBestMoviesOfYear() async {
    ...
    final response = await http.get(Uri.parse('$_baseUrl/discover/movie?api_key=$_apiKey&sort_by=vote_average.desc&year=$year'));
    return _parseMoviesResponse(response);
  }
  ...
  Future<List<Movie>> fetchTrendingMovies({String timeWindow = 'week'}) async {
    final response = await http.get(Uri.parse('$_baseUrl/trending/movie/$timeWindow?api_key=$_apiKey'));
    return _parseMoviesResponse(response);
  }
  ...
  Future<List<Movie>> fetchTVShowsByGenre(String genreName, {int pageCount = 1}) async {
    ...
    List<Movie> allShows = [];
    for (int i = 1; i <= pageCount; i++) {
      final response = await http.get(Uri.parse('$_baseUrl/discover/tv?api_key=$_apiKey&with_genres=$genreId&page=$i'));
      allShows.addAll(_parseMoviesResponse(response, isTV: true));
    }
    return allShows;
  }
  ...
  Future<List<Movie>> fetchMoviesByGenre(String genreName, {int pageCount = 1}) async {
    ...
    List<Movie> allMovies = [];
    for (int i = 1; i <= pageCount; i++) {
      final response = await http.get(Uri.parse('$_baseUrl/discover/movie?api_key=$_apiKey&with_genres=$genreId&page=$i'));
      allMovies.addAll(_parseMoviesResponse(response));
    }
    return allMovies;
  }
  ...
  List<Movie> _parseMoviesResponse(http.Response response, {bool isTV = false}) {
    if (response.statusCode == 200) {
      final data = json.decode(response.body);
      return (data['results'] as List).map((result) => Movie.fromJson(result, isTV: isTV)).toList();
    } else {
